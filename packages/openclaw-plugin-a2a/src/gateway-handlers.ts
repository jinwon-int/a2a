/**
 * A2A gateway method handlers, plugin-local.
 *
 * These handlers own the gateway RPC surface for a2a.task.* methods.
 * They delegate to the standalone a2a-broker HTTP endpoint directly,
 * keeping zero core imports (extension boundary compliant).
 */
import type { GatewayRequestHandlerOptions } from "openclaw/plugin-sdk/gateway-runtime";
import {
  createConfiguredA2ABrokerClient,
  resolveA2ABrokerAdapterPluginConfig,
  type A2ABrokerAdapterPluginRuntimeConfig,
} from "../config.js";
import {
  A2ABrokerClientError,
  buildBrokerCreateTaskRequestFromOpenClaw,
  type A2ABrokerPartyRef,
  type A2ABrokerTaskRecord,
} from "../standalone-broker-client.js";
import {
  mapBrokerErrorToTaskError,
  mapBrokerStatusToDeliveryStatus as mapBrokerDeliveryStatusFromTypeMapping,
  mapBrokerStatusToExecutionStatus as mapBrokerExecutionStatusFromTypeMapping,
  toEpochMs,
} from "../type-mapping.js";
import type {
  A2ATaskApproveParams,
  A2ATaskCancelParams,
  A2ATaskRejectApprovalParams,
  A2ATaskRequestParams,
  A2ATaskStatusParams,
  A2ATaskUpdateParams,
  A2APeerStatusParams,
} from "./gateway-schema.js";
import {
  validateA2ATaskApproveParams,
  validateA2ATaskCancelParams,
  validateA2ATaskRejectApprovalParams,
  validateA2ATaskRequestParams,
  validateA2ATaskStatusParams,
  validateA2ATaskUpdateParams,
  validateA2APeerStatusParams,
  validateParams,
} from "./gateway-validators.js";
import { a2aError, A2AErrorCodes } from "./plugin-errors.js";
import {
  buildTeamAssignmentRequests as normalizeTeamAssignmentRequests,
  type TeamAssignmentBuildResult,
} from "./team-assignment-normalizer.js";

type RawBrokerClient = ReturnType<typeof createConfiguredA2ABrokerClient>;

type BrokerClient = {
  requestTask(params: A2ATaskRequestParams): Promise<unknown>;
  updateTask(params: A2ATaskUpdateParams): Promise<unknown>;
  cancelTask(params: A2ATaskCancelParams): Promise<unknown>;
  approveTask(params: A2ATaskApproveParams): Promise<unknown>;
  rejectApproval(params: A2ATaskRejectApprovalParams): Promise<unknown>;
  statusTask(params: A2ATaskStatusParams): Promise<unknown>;
  peerStatus(params: A2APeerStatusParams): Promise<unknown>;
};

export function createA2AGatewayBrokerClient(
  config: A2ABrokerAdapterPluginRuntimeConfig,
  deps: {
    createRawBrokerClient?: (config: A2ABrokerAdapterPluginRuntimeConfig) => RawBrokerClient;
  } = {},
): BrokerClient {
  const rawClient = deps.createRawBrokerClient?.(config) ?? createConfiguredA2ABrokerClient(config);
  const resolvedConfig = resolveA2ABrokerAdapterPluginConfig(config);

  return {
    async requestTask(params: A2ATaskRequestParams): Promise<unknown> {
      const request = params.request;
      const brokerTask = await rawClient.createTask(
        buildBrokerCreateTaskRequestFromOpenClaw({
          taskId: normalizeString(request.taskId),
          waitRunId: normalizeString(request.runtime?.waitRunId),
          correlationId: normalizeString(request.correlationId),
          parentRunId: normalizeString(request.parentRunId),
          requesterSessionKey:
            normalizeString(request.requester?.sessionKey) ?? normalizeString(params.sessionKey),
          requesterDisplayKey: normalizeString(request.requester?.displayKey),
          requesterChannel: normalizeString(request.requester?.channel),
          targetNodeId:
            normalizeString(request.target.displayKey) ??
            normalizeString(request.target.sessionKey),
          targetSessionKey: request.target.sessionKey,
          targetDisplayKey: request.target.displayKey,
          targetChannel: normalizeString(request.target.channel),
          originalMessage: request.task.instructions,
          taskInput: normalizeUnknownRecord(request.task.input),
          expectedOutput: buildExpectedOutputPayload(request.task.expectedOutput),
          roundOneReply: normalizeString(request.runtime?.roundOneReply),
          announceTimeoutMs:
            request.runtime?.announceTimeoutMs ??
            Math.max(0, (request.constraints?.timeoutSeconds ?? 30) * 1000),
          maxPingPongTurns:
            request.runtime?.maxPingPongTurns ?? request.constraints?.maxPingPongTurns ?? 0,
          cancelTarget: request.runtime?.cancelTarget,
        }),
      );
      return buildGatewayTaskResult("a2a.task.request", brokerTask);
    },

    async updateTask(params: A2ATaskUpdateParams): Promise<unknown> {
      const brokerTask = await getBrokerTask(rawClient, params.update.taskId);
      if (!brokerTask) {
        return undefined;
      }
      const nextTask = await applyBrokerTaskUpdate({
        rawClient,
        brokerTask,
        update: params.update,
        requester: resolvedConfig.requester,
      });
      return buildGatewayTaskResult("a2a.task.update", nextTask);
    },

    async cancelTask(params: A2ATaskCancelParams): Promise<unknown> {
      const brokerTask = await getBrokerTask(rawClient, params.cancel.taskId);
      if (!brokerTask) {
        return undefined;
      }
      const actor = resolvedConfig.requester ??
        brokerTask.requester ?? {
          id: params.sessionKey,
          kind: "session",
          role: "hub",
        };
      const canceledTask = await rawClient.cancelTask(params.cancel.taskId, {
        actor,
        ...(normalizeString(params.cancel.reason)
          ? { reason: normalizeString(params.cancel.reason) }
          : {}),
      });
      return buildGatewayTaskResult("a2a.task.cancel", canceledTask);
    },

    async approveTask(params: A2ATaskApproveParams): Promise<unknown> {
      const brokerTask = await getBrokerTask(rawClient, params.approval.taskId);
      if (!brokerTask) {
        return undefined;
      }
      const actor = resolvedConfig.requester ?? brokerTask.requester ?? {
        id: params.sessionKey,
        kind: "session",
        role: "hub",
      };
      const approvedTask = await rawClient.approveTask(params.approval.taskId, {
        actor,
        ...(normalizeString(params.approval.reason)
          ? { reason: normalizeString(params.approval.reason) }
          : {}),
        ...(normalizeString(params.approval.approvalId)
          ? { approvalId: normalizeString(params.approval.approvalId) }
          : {}),
      });
      return buildGatewayTaskResult("a2a.task.approve", approvedTask);
    },

    async rejectApproval(params: A2ATaskRejectApprovalParams): Promise<unknown> {
      const brokerTask = await getBrokerTask(rawClient, params.approval.taskId);
      if (!brokerTask) {
        return undefined;
      }
      const actor = resolvedConfig.requester ?? brokerTask.requester ?? {
        id: params.sessionKey,
        kind: "session",
        role: "hub",
      };
      const rejectedTask = await rawClient.rejectTaskApproval(params.approval.taskId, {
        actor,
        ...(normalizeString(params.approval.reason)
          ? { reason: normalizeString(params.approval.reason) }
          : {}),
        ...(normalizeString(params.approval.approvalId)
          ? { approvalId: normalizeString(params.approval.approvalId) }
          : {}),
        ...(params.approval.status ? { status: params.approval.status } : {}),
      });
      return buildGatewayTaskResult("a2a.task.reject_approval", rejectedTask);
    },

    async statusTask(params: A2ATaskStatusParams): Promise<unknown> {
      const brokerTask = await getBrokerTask(rawClient, params.taskId);
      if (!brokerTask) {
        return undefined;
      }
      return buildGatewayTaskStatus(brokerTask);
    },

    async peerStatus(params: A2APeerStatusParams): Promise<unknown> {
      return await rawClient.peerStatus({
        target: params.target,
        ...(params.maxCacheAgeMs !== undefined ? { maxCacheAgeMs: params.maxCacheAgeMs } : {}),
      });
    },
  };
}

export function createA2AGatewayHandlers(
  config: A2ABrokerAdapterPluginRuntimeConfig,
  deps: {
    createBrokerClient?: (config: A2ABrokerAdapterPluginRuntimeConfig) => BrokerClient | null;
  } = {},
) {
  let brokerClient: BrokerClient | null | undefined;
  let brokerClientError: Error | undefined;

  function getBrokerClient(): BrokerClient | null {
    if (brokerClient !== undefined) {
      return brokerClient;
    }
    try {
      brokerClient = deps.createBrokerClient?.(config) ?? createA2AGatewayBrokerClient(config);
      brokerClientError = undefined;
    } catch (error) {
      brokerClient = null;
      brokerClientError = error instanceof Error ? error : new Error(String(error));
    }
    return brokerClient;
  }

  function respondBrokerUnavailable(opts: GatewayRequestHandlerOptions): void {
    const message = brokerClientError?.message ?? "a2a broker client not initialized";
    opts.respond(false, undefined, a2aError(A2AErrorCodes.NOT_FOUND, message));
  }

  return {
    handleA2ATaskRequest: async (opts: GatewayRequestHandlerOptions): Promise<void> => {
      const check = validateParams(opts.params, validateA2ATaskRequestParams, "a2a.task.request");
      if (!check.valid) {
        opts.respond(false, undefined, check.error);
        return;
      }
      const broker = getBrokerClient();
      if (!broker) {
        respondBrokerUnavailable(opts);
        return;
      }
      try {
        const result = await broker.requestTask(check.data);
        opts.respond(true, result);
      } catch (err) {
        const error = toGatewayError(err);
        opts.respond(
          false,
          undefined,
          a2aError(A2AErrorCodes.INTERNAL, `a2a.task.request failed: ${error}`),
        );
      }
    },

    handleA2ATaskUpdate: async (opts: GatewayRequestHandlerOptions): Promise<void> => {
      const check = validateParams(opts.params, validateA2ATaskUpdateParams, "a2a.task.update");
      if (!check.valid) {
        opts.respond(false, undefined, check.error);
        return;
      }
      const broker = getBrokerClient();
      if (!broker) {
        respondBrokerUnavailable(opts);
        return;
      }
      try {
        const result = await broker.updateTask(check.data);
        if (result == null) {
          opts.respond(
            false,
            undefined,
            a2aError(A2AErrorCodes.NOT_FOUND, `a2a task not found: ${check.data.update.taskId}`),
          );
          return;
        }
        opts.respond(true, result);
      } catch (err) {
        const error = toGatewayError(err);
        opts.respond(
          false,
          undefined,
          a2aError(A2AErrorCodes.INVALID_REQUEST, `a2a.task.update failed: ${error}`),
        );
      }
    },

    handleA2ATaskCancel: async (opts: GatewayRequestHandlerOptions): Promise<void> => {
      const check = validateParams(opts.params, validateA2ATaskCancelParams, "a2a.task.cancel");
      if (!check.valid) {
        opts.respond(false, undefined, check.error);
        return;
      }
      const broker = getBrokerClient();
      if (!broker) {
        respondBrokerUnavailable(opts);
        return;
      }
      try {
        const result = await broker.cancelTask(check.data);
        if (result == null) {
          opts.respond(
            false,
            undefined,
            a2aError(A2AErrorCodes.NOT_FOUND, `a2a task not found: ${check.data.cancel.taskId}`),
          );
          return;
        }
        opts.respond(true, result);
      } catch (err) {
        const error = toGatewayError(err);
        opts.respond(
          false,
          undefined,
          a2aError(A2AErrorCodes.INVALID_REQUEST, `a2a.task.cancel failed: ${error}`),
        );
      }
    },

    handleA2ATaskStatus: async (opts: GatewayRequestHandlerOptions): Promise<void> => {
      const check = validateParams(opts.params, validateA2ATaskStatusParams, "a2a.task.status");
      if (!check.valid) {
        opts.respond(false, undefined, check.error);
        return;
      }
      const broker = getBrokerClient();
      if (!broker) {
        respondBrokerUnavailable(opts);
        return;
      }
      try {
        const result = await broker.statusTask(check.data);
        if (result == null) {
          opts.respond(
            false,
            undefined,
            a2aError(A2AErrorCodes.NOT_FOUND, `a2a task not found: ${check.data.taskId}`),
          );
          return;
        }
        opts.respond(true, result);
      } catch (err) {
        const error = toGatewayError(err);
        opts.respond(
          false,
          undefined,
          a2aError(A2AErrorCodes.INTERNAL, `a2a.task.status failed: ${error}`),
        );
      }
    },

    handleA2ATaskApprove: async (opts: GatewayRequestHandlerOptions): Promise<void> => {
      const check = validateParams(opts.params, validateA2ATaskApproveParams, "a2a.task.approve");
      if (!check.valid) {
        opts.respond(false, undefined, check.error);
        return;
      }
      const broker = getBrokerClient();
      if (!broker) {
        respondBrokerUnavailable(opts);
        return;
      }
      try {
        const result = await broker.approveTask(check.data);
        if (result == null) {
          opts.respond(
            false,
            undefined,
            a2aError(A2AErrorCodes.NOT_FOUND, `a2a task not found: ${check.data.approval.taskId}`),
          );
          return;
        }
        opts.respond(true, result);
      } catch (err) {
        const error = toGatewayError(err);
        opts.respond(
          false,
          undefined,
          a2aError(A2AErrorCodes.INVALID_REQUEST, `a2a.task.approve failed: ${error}`),
        );
      }
    },

    handleA2ATaskRejectApproval: async (opts: GatewayRequestHandlerOptions): Promise<void> => {
      const check = validateParams(
        opts.params,
        validateA2ATaskRejectApprovalParams,
        "a2a.task.reject_approval",
      );
      if (!check.valid) {
        opts.respond(false, undefined, check.error);
        return;
      }
      const broker = getBrokerClient();
      if (!broker) {
        respondBrokerUnavailable(opts);
        return;
      }
      try {
        const result = await broker.rejectApproval(check.data);
        if (result == null) {
          opts.respond(
            false,
            undefined,
            a2aError(A2AErrorCodes.NOT_FOUND, `a2a task not found: ${check.data.approval.taskId}`),
          );
          return;
        }
        opts.respond(true, result);
      } catch (err) {
        const error = toGatewayError(err);
        opts.respond(
          false,
          undefined,
          a2aError(A2AErrorCodes.INVALID_REQUEST, `a2a.task.reject_approval failed: ${error}`),
        );
      }
    },

    handleA2APeerStatus: async (opts: GatewayRequestHandlerOptions): Promise<void> => {
      const check = validateParams(opts.params, validateA2APeerStatusParams, "a2a.peer.status");
      if (!check.valid) {
        opts.respond(false, undefined, check.error);
        return;
      }
      const broker = getBrokerClient();
      if (!broker) {
        respondBrokerUnavailable(opts);
        return;
      }
      try {
        const result = await broker.peerStatus(check.data);
        opts.respond(true, result);
      } catch (err) {
        const error = toGatewayError(err);
        opts.respond(
          false,
          undefined,
          a2aError(A2AErrorCodes.INTERNAL, `a2a.peer.status failed: ${error}`),
        );
      }
    },
  };
}

export type GatewayTaskMethod =
  | "a2a.task.request"
  | "a2a.task.update"
  | "a2a.task.cancel"
  | "a2a.task.approve"
  | "a2a.task.reject_approval";

export async function getBrokerTask(
  rawClient: RawBrokerClient,
  taskId: string,
): Promise<A2ABrokerTaskRecord | undefined> {
  try {
    return await rawClient.getTask(taskId);
  } catch (error) {
    if (error instanceof A2ABrokerClientError && error.status === 404) {
      return undefined;
    }
    throw error;
  }
}

async function applyBrokerTaskUpdate(params: {
  rawClient: RawBrokerClient;
  brokerTask: A2ABrokerTaskRecord;
  update: A2ATaskUpdateParams["update"];
  requester?: A2ABrokerPartyRef;
}): Promise<A2ABrokerTaskRecord> {
  const executionStatus = params.update.executionStatus;
  if (!executionStatus) {
    return params.brokerTask;
  }

  switch (executionStatus) {
    case "accepted": {
      if (params.brokerTask.status !== "queued") {
        return params.brokerTask;
      }
      return await params.rawClient.claimTask(params.brokerTask.id, {
        workerId: resolveWorkerId(params.brokerTask, params.requester),
      });
    }

    case "running": {
      const claimedTask = await ensureBrokerTaskClaimed(params);
      if (claimedTask.status !== "claimed") {
        return claimedTask;
      }
      return await params.rawClient.startTask(claimedTask.id, {
        workerId: resolveWorkerId(claimedTask, params.requester),
      });
    }

    case "completed": {
      const activeTask = await ensureBrokerTaskStarted(params);
      if (activeTask.status === "succeeded") {
        return activeTask;
      }
      return await params.rawClient.completeTask(activeTask.id, {
        workerId: resolveWorkerId(activeTask, params.requester),
        ...(buildBrokerResult(params.update) ? { result: buildBrokerResult(params.update) } : {}),
      });
    }

    case "failed":
    case "timed_out": {
      const activeTask = await ensureBrokerTaskStarted(params);
      if (activeTask.status === "failed") {
        return activeTask;
      }
      return await params.rawClient.failTask(activeTask.id, {
        workerId: resolveWorkerId(activeTask, params.requester),
        error: buildBrokerError(params.update),
      });
    }

    case "waiting_reply":
    case "waiting_external":
    default:
      return params.brokerTask;
  }
}

async function ensureBrokerTaskClaimed(params: {
  rawClient: RawBrokerClient;
  brokerTask: A2ABrokerTaskRecord;
  requester?: A2ABrokerPartyRef;
}): Promise<A2ABrokerTaskRecord> {
  if (params.brokerTask.status !== "queued") {
    return params.brokerTask;
  }
  return await params.rawClient.claimTask(params.brokerTask.id, {
    workerId: resolveWorkerId(params.brokerTask, params.requester),
  });
}

async function ensureBrokerTaskStarted(params: {
  rawClient: RawBrokerClient;
  brokerTask: A2ABrokerTaskRecord;
  requester?: A2ABrokerPartyRef;
}): Promise<A2ABrokerTaskRecord> {
  const claimedTask = await ensureBrokerTaskClaimed(params);
  if (claimedTask.status !== "claimed") {
    return claimedTask;
  }
  return await params.rawClient.startTask(claimedTask.id, {
    workerId: resolveWorkerId(claimedTask, params.requester),
  });
}

export function resolveWorkerId(brokerTask: A2ABrokerTaskRecord, requester?: A2ABrokerPartyRef): string {
  const workerId =
    normalizeString(requester?.id) ??
    normalizeString(brokerTask.claimedBy) ??
    normalizeString(brokerTask.assignedWorkerId) ??
    normalizeString(brokerTask.targetNodeId);
  if (!workerId) {
    throw new Error(`workerId is required to update broker task ${brokerTask.id}`);
  }
  return workerId;
}

function buildBrokerResult(
  update: A2ATaskUpdateParams["update"],
): Record<string, unknown> | undefined {
  const summary = normalizeString(update.summary);
  const output = normalizeUnknownRecord(update.output);
  if (!summary && output === undefined) {
    return undefined;
  }
  return {
    ...(summary ? { summary } : {}),
    ...(output !== undefined ? { output } : {}),
  };
}

function buildBrokerError(update: A2ATaskUpdateParams["update"]): {
  code?: string;
  message: string;
} {
  const message =
    normalizeString(update.error?.message) ??
    normalizeString(update.summary) ??
    (update.executionStatus === "timed_out" ? "task timed out" : "task failed");
  const code =
    normalizeString(update.error?.code) ??
    (update.executionStatus === "timed_out" ? "timed_out" : undefined);
  return {
    ...(code ? { code } : {}),
    message,
  };
}

export function buildGatewayTaskResult(
  method: GatewayTaskMethod,
  brokerTask: A2ABrokerTaskRecord,
): unknown {
  const status = buildGatewayTaskStatus(brokerTask);
  if (method === "a2a.task.cancel") {
    return {
      method,
      abortStatus: brokerTask.status === "canceled" ? "aborted" : "not-attempted",
      ...status,
    };
  }
  return {
    method,
    ...status,
  };
}

export function buildGatewayTaskStatus(brokerTask: A2ABrokerTaskRecord): Record<string, unknown> {
  const payload = readBrokerTaskPayload(brokerTask);
  const requester = buildRequesterRef(payload, brokerTask);
  const target = buildTargetRef(payload, brokerTask);
  const summary =
    normalizeString(brokerTask.result?.summary) ?? normalizeString(brokerTask.result?.note);
  const output = buildGatewayTaskOutput(brokerTask);
  const error = buildGatewayTaskError(brokerTask);
  const metadata = buildGatewayTaskMetadata(payload, brokerTask);

  return {
    taskId: brokerTask.id,
    ...(payload.correlationId ? { correlationId: payload.correlationId } : {}),
    ...(payload.parentRunId ? { parentRunId: payload.parentRunId } : {}),
    ...(requester ? { requester } : {}),
    target,
    executionStatus:
      brokerTask.status === "canceled"
        ? "cancelled"
        : mapBrokerExecutionStatusFromTypeMapping({
            brokerStatus: brokerTask.status,
            brokerErrorCode: normalizeString(brokerTask.error?.code),
          }),
    deliveryStatus: mapBrokerDeliveryStatusFromTypeMapping(brokerTask.status),
    ...(summary ? { summary } : {}),
    ...(output !== undefined ? { output } : {}),
    ...(error ? { error } : {}),
    ...(metadata ? { metadata } : {}),
    updatedAt: toEpochMs(brokerTask.updatedAt),
    ...(brokerTask.claimedAt ? { startedAt: toEpochMs(brokerTask.claimedAt) } : {}),
    hasHeartbeat: false,
  };
}

function buildRequesterRef(
  payload: ReturnType<typeof readBrokerTaskPayload>,
  brokerTask: A2ABrokerTaskRecord,
) {
  const sessionKey =
    payload.requesterSessionKey ??
    (brokerTask.requester.kind === "session" ? brokerTask.requester.id : undefined);
  if (!sessionKey) {
    return undefined;
  }
  return {
    sessionKey,
    displayKey: payload.requesterDisplayKey ?? sessionKey,
    ...(payload.requesterChannel ? { channel: payload.requesterChannel } : {}),
  };
}

function buildTargetRef(
  payload: ReturnType<typeof readBrokerTaskPayload>,
  brokerTask: A2ABrokerTaskRecord,
) {
  const sessionKey = payload.targetSessionKey ?? brokerTask.target.id;
  return {
    sessionKey,
    displayKey: payload.targetDisplayKey ?? sessionKey,
    ...(payload.targetChannel ? { channel: payload.targetChannel } : {}),
  };
}

export function buildGatewayTaskOutput(brokerTask: A2ABrokerTaskRecord): unknown {
  const result = brokerTask.result;
  if (!result) {
    return undefined;
  }
  const output: Record<string, unknown> = {
    ...normalizeUnknownRecord(result.output),
    ...(normalizeString(result.note) ? { note: normalizeString(result.note) } : {}),
    ...(result.artifactIds?.length ? { artifactIds: result.artifactIds } : {}),
    ...(result.validation ? { validation: result.validation } : {}),
    ...(result.apply ? { apply: result.apply } : {}),
    status: brokerTask.status,
  };
  return Object.keys(output).length > 0 ? output : undefined;
}

export function buildGatewayTaskError(
  brokerTask: A2ABrokerTaskRecord,
): { code: string; message?: string } | undefined {
  return mapBrokerErrorToTaskError({
    brokerErrorCode: normalizeString(brokerTask.error?.code),
    brokerErrorMessage: normalizeString(brokerTask.error?.message),
    brokerStatus: brokerTask.status,
  });
}

export function mapBrokerStatusToExecutionStatus(
  brokerTask: A2ABrokerTaskRecord,
): ReturnType<typeof mapBrokerExecutionStatusFromTypeMapping> {
  return mapBrokerExecutionStatusFromTypeMapping({
    brokerStatus: brokerTask.status,
    brokerErrorCode: normalizeString(brokerTask.error?.code),
  });
}

export function mapBrokerStatusToDeliveryStatus(
  status: A2ABrokerTaskRecord["status"],
): ReturnType<typeof mapBrokerDeliveryStatusFromTypeMapping> {
  return mapBrokerDeliveryStatusFromTypeMapping(status);
}

export function readBrokerTaskPayload(brokerTask: A2ABrokerTaskRecord): {
  requesterSessionKey?: string;
  requesterDisplayKey?: string;
  requesterChannel?: string;
  targetSessionKey?: string;
  targetDisplayKey?: string;
  targetChannel?: string;
  correlationId?: string;
  parentRunId?: string;
  taskInput?: Record<string, unknown>;
  expectedOutput?: Record<string, unknown>;
  evidenceRefs?: string[];
} {
  const payload = brokerTask.payload;
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    return {};
  }
  const record = payload;
  return {
    requesterSessionKey: readOptionalRecordString(record, "requesterSessionKey"),
    requesterDisplayKey: readOptionalRecordString(record, "requesterDisplayKey"),
    requesterChannel: readOptionalRecordString(record, "requesterChannel"),
    targetSessionKey: readOptionalRecordString(record, "targetSessionKey"),
    targetDisplayKey: readOptionalRecordString(record, "targetDisplayKey"),
    targetChannel: readOptionalRecordString(record, "targetChannel"),
    correlationId: readOptionalRecordString(record, "correlationId"),
    parentRunId: readOptionalRecordString(record, "parentRunId"),
    taskInput: readOptionalRecord(record, "taskInput"),
    expectedOutput: readOptionalRecord(record, "expectedOutput"),
    evidenceRefs: readOptionalStringArray(record, "evidenceRefs"),
  };
}

function buildExpectedOutputPayload(
  expectedOutput: A2ATaskRequestParams["request"]["task"]["expectedOutput"],
): { format: "text" | "json"; schemaName?: string } | undefined {
  if (!expectedOutput) {
    return undefined;
  }
  return {
    format: expectedOutput.format,
    ...(normalizeString(expectedOutput.schemaName)
      ? { schemaName: normalizeString(expectedOutput.schemaName) }
      : {}),
  };
}

function buildGatewayTaskMetadata(
  payload: ReturnType<typeof readBrokerTaskPayload>,
  brokerTask: A2ABrokerTaskRecord,
): Record<string, unknown> | undefined {
  const mergeGate = buildGitHubMergeGateMetadata(payload, brokerTask);
  const compactTaskInput = buildCompactTaskInputMetadata(payload.taskInput);
  const metadata: Record<string, unknown> = {
    ...(normalizeString(brokerTask.exchangeId) ? { exchangeId: normalizeString(brokerTask.exchangeId) } : {}),
    ...(normalizeString(brokerTask.proposalId) ? { proposalId: normalizeString(brokerTask.proposalId) } : {}),
    ...(brokerTask.policyContext ? { policyContext: brokerTask.policyContext } : {}),
    ...(brokerTask.approval ? { approval: brokerTask.approval } : {}),
    ...(brokerTask.approvalOutcome ? { approvalOutcome: brokerTask.approvalOutcome } : {}),
    ...(mergeGate ? { githubMergeGate: mergeGate } : {}),
    ...(payload.evidenceRefs?.length ? { evidenceRefs: payload.evidenceRefs } : {}),
    ...(payload.expectedOutput ? { expectedOutput: payload.expectedOutput } : {}),
    ...(compactTaskInput ? { taskInput: compactTaskInput } : {}),
  };
  return Object.keys(metadata).length > 0 ? metadata : undefined;
}

const COMPACT_TASK_INPUT_STRING_KEYS = [
  "exchangeId",
  "proposalId",
  "runnerPreset",
  "repository",
  "commitSha",
  "round",
  "prUrl",
  "issueUrl",
  "artifactUrl",
  "doneUrl",
  "blockUrl",
] as const;

function buildCompactTaskInputMetadata(
  taskInput: Record<string, unknown> | undefined,
): Record<string, unknown> | undefined {
  if (!taskInput) {
    return undefined;
  }

  const metadata: Record<string, unknown> = {};
  for (const key of COMPACT_TASK_INPUT_STRING_KEYS) {
    const value = readOptionalRecordString(taskInput, key);
    if (value) {
      metadata[key] = value;
    }
  }

  const artifactIds = readOptionalStringArray(taskInput, "artifactIds");
  if (artifactIds?.length) {
    metadata.artifactIds = artifactIds;
  }

  return Object.keys(metadata).length > 0 ? metadata : undefined;
}

type GitHubMergeGateState = "clean" | "review_required" | "conflict" | "blocked" | "unknown";

function buildGitHubMergeGateMetadata(
  payload: ReturnType<typeof readBrokerTaskPayload>,
  brokerTask: A2ABrokerTaskRecord,
): Record<string, unknown> | undefined {
  const source =
    findMergeGateRecord(payload.taskInput) ??
    findMergeGateRecord(normalizeUnknownRecord(brokerTask.result?.output)) ??
    findMergeGateRecord(normalizeUnknownRecord(brokerTask.error?.details));
  if (!source) {
    return undefined;
  }

  const mergeStateStatus = readOptionalRecordString(source, "mergeStateStatus");
  const reviewDecision = readOptionalRecordString(source, "reviewDecision");
  const requiredReviewCount = readOptionalRecordNumber(source, "requiredReviewCount");
  const requireLastPushApproval = readOptionalRecordBoolean(source, "requireLastPushApproval");
  const hasConflicts = readOptionalRecordBoolean(source, "hasConflicts");
  const summary = readOptionalRecordString(source, "summary");
  const blockedBy = readOptionalStringArray(source, "blockedBy");
  const state = deriveGitHubMergeGateState({
    mergeStateStatus,
    reviewDecision,
    requiredReviewCount,
    requireLastPushApproval,
    hasConflicts,
    blockedBy,
  });

  const metadata: Record<string, unknown> = {
    state,
    ...(mergeStateStatus ? { mergeStateStatus } : {}),
    ...(reviewDecision ? { reviewDecision } : {}),
    ...(requiredReviewCount !== undefined ? { requiredReviewCount } : {}),
    ...(requireLastPushApproval !== undefined ? { requireLastPushApproval } : {}),
    ...(hasConflicts !== undefined ? { hasConflicts } : {}),
    ...(blockedBy?.length ? { blockedBy } : {}),
    ...(summary ? { summary } : {}),
  };
  return Object.keys(metadata).length > 1 || state !== "unknown" ? metadata : undefined;
}

function findMergeGateRecord(record: Record<string, unknown> | undefined): Record<string, unknown> | undefined {
  if (!record) {
    return undefined;
  }
  return (
    readOptionalRecord(record, "githubMergeGate") ??
    readOptionalRecord(record, "mergeGate") ??
    readOptionalRecord(readOptionalRecord(record, "github") ?? {}, "mergeGate") ??
    readOptionalRecord(readOptionalRecord(record, "metadata") ?? {}, "mergeGate")
  );
}

function deriveGitHubMergeGateState(params: {
  mergeStateStatus?: string;
  reviewDecision?: string;
  requiredReviewCount?: number;
  requireLastPushApproval?: boolean;
  hasConflicts?: boolean;
  blockedBy?: string[];
}): GitHubMergeGateState {
  const mergeStateStatus = params.mergeStateStatus?.toUpperCase();
  const reviewDecision = params.reviewDecision?.toUpperCase();
  if (params.hasConflicts === true || mergeStateStatus === "DIRTY" || mergeStateStatus === "UNKNOWN") {
    return "conflict";
  }
  if (
    reviewDecision === "REVIEW_REQUIRED" ||
    (params.requiredReviewCount !== undefined && params.requiredReviewCount > 0) ||
    params.requireLastPushApproval === true ||
    params.blockedBy?.some((blocker) => /review|approval/i.test(blocker))
  ) {
    return "review_required";
  }
  if (["BLOCKED", "BEHIND", "HAS_HOOKS", "UNSTABLE"].includes(mergeStateStatus ?? "")) {
    return "blocked";
  }
  if (mergeStateStatus === "CLEAN" || reviewDecision === "APPROVED") {
    return "clean";
  }
  return "unknown";
}

function readOptionalRecordString(
  record: Record<string, unknown>,
  key: string,
): string | undefined {
  const value = record[key];
  return typeof value === "string" && value.trim() ? value.trim() : undefined;
}

function readOptionalRecord(
  record: Record<string, unknown>,
  key: string,
): Record<string, unknown> | undefined {
  const value = record[key];
  return value && typeof value === "object" && !Array.isArray(value)
    ? (value as Record<string, unknown>)
    : undefined;
}

function readOptionalRecordNumber(
  record: Record<string, unknown>,
  key: string,
): number | undefined {
  const value = record[key];
  return typeof value === "number" && Number.isFinite(value) ? value : undefined;
}

function readOptionalRecordBoolean(
  record: Record<string, unknown>,
  key: string,
): boolean | undefined {
  const value = record[key];
  return typeof value === "boolean" ? value : undefined;
}

function readOptionalStringArray(
  record: Record<string, unknown>,
  key: string,
): string[] | undefined {
  const value = record[key];
  if (!Array.isArray(value)) {
    return undefined;
  }
  const normalized = value
    .map((item) => (typeof item === "string" && item.trim() ? item.trim() : undefined))
    .filter((item): item is string => Boolean(item));
  return normalized.length > 0 ? normalized : undefined;
}

export function normalizeString(value: string | undefined): string | undefined {
  const trimmed = value?.trim();
  return trimmed ? trimmed : undefined;
}

export function normalizeUnknownRecord(value: unknown): Record<string, unknown> | undefined {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return undefined;
  }
  return value as Record<string, unknown>;
}

function toGatewayError(error: unknown): string {
  return error instanceof Error ? error.message : String(error);
}

/**
 * Convert a raw team assignment intent into one or more validated
 * a2a.task.request params. Gateway-facing wrapper around the normalizer
 * that maps validator errors onto the plugin's a2aError shape.
 */
export function buildTeamAssignmentRequests(
  input: unknown,
):
  | { ok: true; requests: A2ATaskRequestParams[] }
  | { ok: false; error: { code: string; message: string } } {
  const result: TeamAssignmentBuildResult = normalizeTeamAssignmentRequests(input);
  if (!result.ok) {
    return {
      ok: false,
      error: a2aError(
        A2AErrorCodes.INVALID_REQUEST,
        `${result.error.code}: ${result.error.message}`,
      ),
    };
  }
  return { ok: true, requests: result.requests };
}
