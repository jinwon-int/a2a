...
test('Team1/yukson Terminal Brief activation libero covers run and sibling lanes', async () => {
...
test('Team1/yukson activation matrix fails closed on Start-only evidence', async () => {
  const content = await doc();
  ...
  assert.match(content, /Start evidence only/);
...
test('Team1/yukson activation matrix documents deploy config canary receipt gates', async () => {
  const content = await doc();
